...
<div class="notifications view large-9 medium-8 columns content">
    ...
    <table class="vertical-table">
        ...
        <tr>
            <th scope="row"><?= __('Idtopic') ?></th>
            <td><?= $this->Number->format($notification->idtopic) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('User') ?></th>
            <td><?= $this->Number->format($notification->user) ?></td>
        </tr>
        <tr>
    ...
    <div class="row">
        <h4><?= __('Type') ?></h4>
        <?= $this->Text->autoParagraph(h($notification->type)); ?>
    </div>
    <div class="row">
        <h4><?= __('Cate') ?></h4>
        <?= $this->Text->autoParagraph(h($notification->cate)); ?>
    </div>
    <div class="row">
        <h4><?= __('Content') ?></h4>
